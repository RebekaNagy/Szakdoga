...
checkP4 :: [IdEnvironment] -> Program -> SideCondition -> Int -> [IdEnvironment]
checkP4 environmentList program sideconditions number = 
...
prFunc_Action envlist name program sideconditions number = 
    (checkP4 (map (\(id, envtype, env) -> (id++"$"++(show number)++"action:"++name, envtype, env)) envlist) program sideconditions (number+1))
...
prFunc_Table envlist name keys (action:acts) sideconditions@(SideCon (_, tableCondition, _, _, _)) number = 
    newStuckEnvlist ++ (checkP4 newFilteredEnvlist action sideconditions number) ++ (prFunc_Table filteredEnvlist name keys acts sideconditions number) 
